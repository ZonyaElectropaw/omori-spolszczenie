...
Window_BattleLog.prototype.makeCustomActionText = function(subject, target, item) {
  ...
  /**function genderbender(kto, corobi) { < debilny kod jest debilny
    if(kto === "AUBREY") {
      switch(corobi) {
      case ' stracił ':
        czyn = " straciła ";
 ...
 * @param {string} corobi 
 */
  function genderbender_experimental(kto, corobi){
    if(kto === "AUBREY"){
      if(corobi.slice(-1) === 'ł'){
        return corobi.slice(0, corobi.length - 1) + "ła";
      }
      if(corobi.slice(-1) === 'Y'){
        return corobi.slice(0, corobi.length - 1) + 'A';
      } 
      else return corobi;
    }else return corobi;
  }
...
if (hpDam != 0) {
  ...
  if (strongHit) {
    hpDamageText = '...To był poruszający atak!\r\n' + hpDamageText;
  } else if (weakHit) {
    hpDamageText = '...To był nieciekawy atak.\r\n' + hpDamageText;
  }
} else if (result.isHit() === true) {
  var hpDamageText = "atak " + user.name() + " nic nie zrobił.";
} else {
  var hpDamageText = "atak " + user.name() + " chybił!";
}

if (critical) {
    hpDamageText = 'PROSTO W SERCE!\r\n' + hpDamageText;
}

if (mpDam > 0) {
  var mpDamageText = target.name() + " " + genderbender_experimental(target.name(), "stracił") + " " + mpDam + ' SOKU...';
  ...
  case 'BLANK': // ATTACK
    ...
    case 'ATTACK': // ATTACK
    czyn = ' atakuje '
    text = user.name() + czyn + target.name() + '!\r\n';
    text += hpDamageText;
    break;

  case 'MULTIHIT':
    text = user.name() + "robi poruszający atak!\r\n";
    break;

  case 'OBSERVE': // OBSERVE
    text = user.name() + ' skupia swój wzrok i obserwuje.\r\n';
    text += target.name() + '!';
    break;

  case 'OBSERVE TARGET': // OBSERVE TARGET
    //text = user.name() + " observes " + target.name() + ".\r\n";
    text = target.name() + ' ma na oku\r\n';
    text += user.name() + '!';
    break;
  ...
  case 'OBSERVE ALL': // OBSERVE TARGET
    ...
    text = user.name() + ' skupia swój wzrok i obserwuje.\r\n';
    text += target.name() + '!';
    text = target.name() + ' ma na oku wszystkich!';
    break;
  ...
  case 'SAD POEM':  // SAD POEM
    ...
    if(!target._noEffectMessage) {
      if(target.isStateAffected(12)) {
        text += target.name() + ' czuje się ' + genderbender_experimental(target.name(), "NĘDZNY") + '...';
      }
      
      else if(target.isStateAffected(11)) {
        text += target.name() + ' czuje się ' + genderbender_experimental(target.name(), "DEPRESYJNY") + '..';
      }
      
      else if(target.isStateAffected(10)) {
        text += target.name() + ' czuje się ' + genderbender_experimental(target.name(), "SMUTNY") + '.';
      }
    }
    else {
      text += parseNoEffectEmotion(target.name(), genderbender_experimental(target.name(), czyn) + '!')}
    break;
  ...
  case 'TRUTH': // PAINFUL TRUTH
    ...
    text += hpDamageText + "\r\n";
    if(!target._noEffectMessage) {
      text += target.name() + " czuje się SAD.\r\n";
    }
    else {text += parseNoEffectEmotion(target.name(), "SADDER!\r\n")}
    if(user.isStateAffected(12)) {text += user.name() + " czuje się MISERABLE...";}
    else if(user.isStateAffected(11)) {text += user.name() + " czuje się DEPRESSED..";}
    else if(user.isStateAffected(10)) {text += user.name() + " czuje się SAD.";}
    break;
  ...
  case 'TRIP':  // TRIP
    ...
    case 'TRIP 2':  // TRIP 2
      ...
      if(!target._noEffectMessage) {text += target.name() + ' czuje się SAD.\r\n';}
      else {text += parseNoEffectEmotion(target.name(), "SADDER!\r\n")}
      text += hpDamageText;
      break;
  ...
  case 'STARE': // STARE
    ...
    text += target.name() + ' czuje się uncomfortable.';
    break;
  ...
  case 'PEP TALK':  // PEP TALK
    ...
    if(!target._noEffectMessage) {
      if(target.isStateAffected(8)) {text += target.name() + ' czuje się MANIC!!!';}
      else if(target.isStateAffected(7)) {text += target.name() + ' czuje się ECSTATIC!!';}
      else if(target.isStateAffected(6)) {text += target.name() + ' czuje się HAPPY!';}
    }
  ...
  case 'TEAM SPIRIT':  // TEAM SPIRIT
    ...
    if(!user._noEffectMessage) {
      if(user.isStateAffected(8)) {text += user.name() + ' czuje się MANIC!!!';}
      else if(user.isStateAffected(7)) {text += user.name() + ' czuje się ECSTATIC!!';}
      else if(user.isStateAffected(6)) {text += user.name() + ' czuje się HAPPY!';}
    }
  ...
  case 'LOOK KEL 1':  // Look at Kel 1
    ...
    text += target.name() + " czuje się ANGRY!";
    break;
  ...
  case 'LOOK KEL 2': // Look at Kel 2
   ...
   if(AUBREY.isStateAffected(14) && KEL.isStateAffected(14)) {text += 'KEL and AUBREY feel ANGRY!';}
    ...
    text += 'KEL czuje się ENRAGED!!\r\n';
    text += 'AUBREY czuje się ANGRY!';
   }
   else if(AUBREY.isStateAffected(15) && KEL.isStateAffected(14)) {
    text += 'KEL czuje się ANGRY!\r\n';
    text += 'AUBREY czuje się ENRAGED!!';
   }
  ...
  case 'LOOK HERO':  // LOOK AT HERO 1
    ...
    if(target.isStateAffected(6)) {text += target.name() + " czuje się HAPPY!\r\n"}
    else if(target.isStateAffected(7)) {text += target.name() + " czuje się ECSTATIC!!\r\n"}
  ...
  case 'TWIRL': // ATTACK
    text = user.name() + ' atakuje ' + target.name() + '!\r\n';
    text += hpDamageText;
    break;
  ...
  //KEL//
    case 'ANNOY':  // ANNOY
      ...
      if(!target._noEffectMessage) {
        if(target.isStateAffected(14)) {text += target.name() + ' czuje się ANGRY!';}
        else if(target.isStateAffected(15)) {text += target.name() + ' czuje się ENRAGED!!';}
        else if(target.isStateAffected(16)) {text += target.name() + ' czuje się FURIOUS!!!';}
      }
    ...
    case 'FLEX':  // FLEX
      text = user.name() + ' flexes and czuje się his best!\r\n';
    ...
    case 'RALLY': // RALLY
      ...
      if(user.isStateAffected(7)) {text += user.name() + " czuje się ECSTATIC!!\r\n"}
      else if(user.isStateAffected(6)) {text += user.name() + " czuje się HAPPY!\r\n"}
    ...
    case 'CURVEBALL': // CURVEBALL
     ...
     switch($gameTemp._randomState) {
       case 6:
         if(!target._noEffectMessage) {text += target.name() + " czuje się HAPPY!\r\n"}
         else {text += parseNoEffectEmotion(target.name(), "HAPPIER!\r\n")}
         break;
      case 14:
        if(!target._noEffectMessage) {text += target.name() + " czuje się ANGRY!\r\n"}
        else {text += parseNoEffectEmotion(target.name(), "ANGRIER!\r\n")}
        break;
      case 10:
        if(!target._noEffectMessage) {text += target.name() + " czuje się SAD.\r\n"}
    ...
    case 'MEGAPHONE': // MEGAPHONE
      ...
      if(target.isStateAffected(16)) {text += target.name() + ' czuje się FURIOUS!!!\r\n'}
      else if(target.isStateAffected(15)) {text += target.name() + ' czuje się ENRAGED!!\r\n'}
      else if(target.isStateAffected(14)) {text += target.name() + ' czuje się ANGRY!\r\n'}
      break;
    ...
    case 'PASS OMORI 2': //KEL PASS OMORI 2
      ...
      if(OMORI.isStateAffected(6)) {text += "OMORI czuje się HAPPY!\r\n"}
      else if(OMORI.isStateAffected(7)) {text += "OMORI czuje się ECSTATIC!!\r\n"}
      text += hpDamageText;
      break;
    ...
    case 'DAZZLE':
      ...
      if(!target._noEffectMessage) {
        text += target.name() + ' czuje się HAPPY!';
      }
      else {text += parseNoEffectEmotion(target.name(), "HAPPIER!")}
      break;
    ...
    case 'TEA TIME': // TEA TIME
      ...
      text += target.name() + ' czuje się refreshed!\r\n';
    ...
    case 'SUPER TAUNT': // SUPER TAUNT
      text = user.name() + ' draws the foe\'s attention.\r\n';
      text += user.name() + ' prepares to block enemy atakuje.';
      break;
  ...
  //UNIVERSAL//
    ...
    case 'GAURD': // GAURD
      text = user.name() + ' prepares to block atakuje.';
      break;
  ...
  //FOREST BUNNY?//
    ...
    case 'STRANGE BEAM':  // STRANGE BEAM
      ...
      text += target.name() + " czuje się a random EMOTION!"
      break;
    ...
    case 'SSS SLITHER': //STRAWBERRY SHORT SNAKE SLITHER
      ...
      if(!user._noEffectMessage) {text += user.name() + ' czuje się HAPPY!';}
      else {text += parseNoEffectEmotion(user.name(), "HAPPIER!")}
      break;
    ...
    case 'MUFFLED SCREAMS': //MUFFLED SCREAMS
      ...
      if(!target._noEffectMessage && target.name() !== "OMORI") {
        text += target.name() + ' czuje się AFRAID.';
      }
    ...
    case 'ROAR': //ROAR
      ...
      if(!user._noEffectMessage) {text += user.name() + ' czuje się ANGRY!';}
    ...
    case 'MAFIA NOTHING': //MAFIA NOTHING
      text = user.name() + ' strzela knykciami.';
      break;
    ...
    case 'MUSSEL FLEX': //MUSSEL FLEX
     text = user.name() + ' flexes and czuje się its best!\r\n';
    ...
    case 'SHARK WORK UP': //SHARK FIN WORK UP
      ...
      if(!user._noEffectMessage) {
        text += user.name() + ' czuje się ANGRY!';
      }
      else {text += parseNoEffectEmotion(user.name(), "ANGRIER!")}
      break;
    ...
    case 'SLIME BUN STICKY': //SLIME BUN STICKY
      text = user.name() + ' czuje się lonely and cries.\r\n';
      if(!target._noStateMessage) {text += target.name() + '\'s SPEED fell!\r\n';}
      else {text += parseNoStateChange(target.name(), "SPEED", "lower!\r\n")}
      text += target.name() + " czuje się SAD.";
      break;
    ...
    case 'MERCI ATTACK': //MERCI ATTACK
      ...
      text += target.name() + ' czuje się their organs being torn!\r\n';
      text += hpDamageText;
      break;
    ...
    case 'MERCI MELODY': //MERCI LAUGH
      ...
      else if(target.isStateAffected(8)) {text += target.name() + " czuje się MANIC!!!\r\n"}
      break;
    ...
    case 'GHOST BUNNY ATTACK': //GHOST BUNNY ATTACK
      ...
      text += target.name() + ' czuje się tired.\r\n';
      text += mpDamageText;
      break;
    ...
    case 'GINGER ATTACK': //GINGER ATTACK
      text = user.name() + ' snaps and atakuje ' + target.name() + '.\r\n';
      text += hpDamageText;
      break;
    ...
    case 'REPAIR':  // REPAIR
      ...
      text += 'KID\'S KITE czuje się good as new!';
      break;
    ...
    case 'PLUTO BRAG':  // PLUTO BRAG
      ...
      if(!user._noEffectMessage) {
        text += user.name() + ' czuje się HAPPY!';
      }
      else {text += parseNoEffectEmotion(user.name(), "HAPPIER!")}
      break;
    ...
    case 'ANGSTY SONG':  // ANGSTY SONG
      ...
      if(target.isStateAffected(10)) {text += target.name() + ' czuje się SAD.';}
      else if(target.isStateAffected(11)) {text += target.name() + ' czuje się DEPRESSED..';}
      else if(target.isStateAffected(12)) {text += target.name() + ' czuje się MISERABLE...';}
      break;
    ...
    case 'SH BRAG': //SWEET HEART BRAG
      ...
      case 'DESK NOTHING': //MR. JAWSUM DESK DO NOTHING
        text = user.name() + ' begins counting CLAMSÓW.';
        break;
      ...
      case 'TENTACLE GRAB': //ABBI TENTACLE GRAB
        ...
        if(result.isHit()) {
          if(target.name() !== "OMORI" && !target._noEffectMessage) {text += target.name() + " czuje się AFRAID.\r\n";}
          else {text += parseNoEffectEmotion(target.name(), "AFRAID")}
        }
        text += hpDamageText;
        break;
      ...
      case 'TENTACLE GOOP': //ABBI TENTACLE GOOP
        ...
        text += target.name() + ' czuje się weaker...\r\n';
        text += target.name() + '\'s ATTACK fell.\r\n';
      ...
      case 'MUTANT HEART KILL': //MUTANT HEART KILL
        ...
        case 'PERFECT ANGELIC VOICE': //UPLIFTING HYMN
          if(target.index() <= unitLowestIndex) {
            ...
            if(!user._noEffectMessage) {text += user.name() + " czuje się SAD.\r\n"}
        ...
        case "PERFECT ANGELIC WRATH":
          ...
          if(!target._noEffectMessage) {
              ...
              else if(target.isStateAffected(10)) {text += target.name() + ' czuje się SAD.\r\n';}
              else if(target.isStateAffected(12)) {text += target.name() + ' czuje się FURIOUS!!!\r\n';}
              else if(target.isStateAffected(11)) {text += target.name() + ' czuje się ENRAGED!!\r\n';}
              else if(target.isStateAffected(10)) {text += target.name() + ' czuje się ANGRY!\r\n';}
          }
          else {
      ...
      case 'H SWARM ATTACK': //HUMPHREY SWARM ATTACK
        text = 'HUMPHREY surrounds and atakuje ' + target.name() + '!\r\n';
        text += hpDamageText;
        break;
    ...
    case 'DREAM HEIGHTS SHOVE': //DREAM FEAR OF HEIGHTS SHOVE
      text = user.name() + ' shoves ' + target.name() + '.\r\n';
      text += hpDamageText + '\r\n';
      if(!target._noEffectMessage && target.name() !== "OMORI"){text += target.name() + ' czuje się AFRAID.';}
      else {text += parseNoEffectEmotion(target.name(), "AFRAID")}
      break;
    ...
    case 'DREAM DROWNING BIG': //DREAM FEAR OF DROWNING BIG
      text = 'Everyone czuje się like passing out.';
      break;
    ...
    //BERLY//
      ...
      case 'DANDELION':  // DANDELION
        ...
        text += user.name() + ' czuje się like ' + (switches.value(6) ? 'her' : 'his') + 'self again.';
        break;
      ...
      case 'COFFEE': // COFFEE
        ...
        text += user.name() + ' czuje się amazing!';
        break;
      ...
      case 'ECSTATIC':
        if(!target._noEffectMessage){text = target.name() + ' czuje się ECSTATIC!!';}
        else {text = parseNoEffectEmotion(target.name(), "HAPPIER!")}
        break;

      case 'MANIC':
        if(!target._noEffectMessage){text = target.name() + ' czuje się MANIC!!!';}
        else {text = parseNoEffectEmotion(target.name(), "HAPPIER!")}
        break;
      ...
      case 'DEPRESSED':
        if(!target._noEffectMessage){text = target.name() + ' czuje się DEPRESSED..';}
        else {text = parseNoEffectEmotion(target.name(), "SADDER!")}
        break;

      case 'MISERABLE':
        if(!target._noEffectMessage){text = target.name() + ' czuje się MISERABLE...';}
        else {text = parseNoEffectEmotion(target.name(), "SADDER!")}
        break;
      ...
      case 'ENRAGED':
        if(!target._noEffectMessage){text = target.name() + ' czuje się ENRAGED!!';}
        else {text = parseNoEffectEmotion(target.name(), "ANGRIER!")}
        break;

      case 'FURIOUS':
        if(!target._noEffectMessage){text = target.name() + ' czuje się FURIOUS!!!'}
        else {text = parseNoEffectEmotion(target.name(), "ANGRIER!")}
        break;

      case 'AFRAID':
        if(!target._noEffectMessage){text = target.name() + ' czuje się AFRAID!';}
        else {text = parseNoEffectEmotion(target.name(), "AFRAID")}
        break;
