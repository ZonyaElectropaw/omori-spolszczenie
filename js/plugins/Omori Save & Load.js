...
Scene_OmoriFile.prototype.onSelectInputOk = function() {
  ...
  if (index === 0) {
    ...
    if (StorageManager.exists(saveFileid)) {
      // Show Prompt Window
      this.showPromptWindow('Nadpisać ten plik?');
    ...
    if (StorageManager.exists(saveFileid)) {
      // Show Prompt Window
      this.showPromptWindow('Załadować ten plik?');
      // Set Can select Flag to false
      this._canSelect = false;
    } else {
      // Play Buzzer Sound
...
Window_OmoriFileInformation.prototype.refresh = function() {
  ...
  this.contents.drawText('PLIK ' + id + ':', 10 + 30, -5, 100, this.contents.fontSize);
  // If Valid
  if (valid) {
    ...
    // Draw Level
    this.contents.drawText('POZIOM:', 270 + 55, 30, 100, 24);
    this.contents.drawText(actor.level, 290 + 55, 30, 70, 24, 'right');
    // Draw Total PlayTime
    this.contents.drawText('CZAS GRY:', 118, 55, 200, 24);
    this.contents.drawText(info.playtime, 295 + 55, 55, 100, 24);
    // Draw Location
    this.contents.drawText('LOKALIZACJA:', 118, 80, 200, 24);
...
Window_OmoriFileCommand.prototype.makeCommandList = function () {
  this.addCommand("ZAPISZ", 'save', this._canSave);
  this.addCommand("ŁADUJ", 'load', this._canLoad);
};
...
Window_OmoriFilePrompt.prototype.makeCommandList = function () {
  this.addCommand("TAK", 'ok');
  this.addCommand("NIE", 'cancel');
};
//=============================================================================
// * Refresh
//=============================================================================
